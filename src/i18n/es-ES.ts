
export const esES = {
  common: {
    language: 'Español',
    switchTo: 'Cambiar a Inglés',
  },
  profile: {
    title: 'Asesor Financiero',
    location: 'Ubicación',
    contact: 'Contáctame',
    schedule: 'Programar Reunión',
    import: 'Importar de LinkedIn',
    share: 'Compartir Perfil',
  },
  tabs: {
    about: 'Acerca de',
    rating: 'Calificación',
    experience: 'Experiencia',
    education: 'Educación',
    testimonials: 'Testimonios',
    custom: 'Personalizado',
    contact: 'Contacto',
  },
  rating: {
    title: 'Calificación del Asesor',
    basedOn: 'Basado en',
    reviews: 'opiniones de clientes',
    description: 'Las calificaciones reflejan la satisfacción de los clientes con los servicios del asesor',
  },
  sections: {
    summary: {
      title: 'Resumen Profesional',
      placeholder: 'Comparte tu experiencia profesional, conocimientos y enfoque para trabajar con clientes...',
    },
    services: {
      title: 'Servicios Ofrecidos',
      addService: 'Añadir Servicio',
      specialization: 'Áreas de Especialización',
      addSpecialization: 'Añadir Especialización',
    },
    experience: {
      title: 'Experiencia Laboral',
      present: 'Presente',
      addExperience: 'Añadir Experiencia Laboral',
    },
    education: {
      title: 'Educación',
      addEducation: 'Añadir Educación',
      certifications: 'Certificaciones',
      addCertification: 'Añadir Certificación',
    },
    testimonials: {
      title: 'Testimonios de Clientes',
      addTestimonial: 'Añadir Testimonio de Cliente',
      description: 'Permite que tus clientes compartan sus experiencias',
    },
    custom: {
      title: 'Mensaje Personal',
      placeholder: 'Comparte un mensaje personal con tus clientes...',
    },
  },
  customer: {
    writeReview: 'Escribir una Reseña',
    yourRating: 'Tu Calificación',
    yourName: 'Tu Nombre',
    company: 'Empresa (Opcional)',
    yourReview: 'Tu Reseña',
    reviewPlaceholder: 'Comparte tu experiencia con este asesor...',
    submit: 'Enviar Reseña',
    submitting: 'Enviando...',
    thankYou: '¡Gracias por tus comentarios!',
    errorMessage: 'Por favor complete los campos requeridos',
  },
  advisor: {
    editProfile: 'Editar Perfil de Asesor',
    saveProfile: 'Guardar Perfil',
    backToPublic: 'Volver a Vista Pública',
    linkedinImport: 'Importar de LinkedIn',
    profileSaved: 'Perfil guardado',
    profileSavedDesc: '¡Tu perfil ha sido actualizado correctamente!',
    importingLinkedIn: 'Estamos obteniendo tus datos de perfil de LinkedIn...',
    importComplete: 'Importación de LinkedIn completada',
    importCompleteDesc: '¡Tu perfil ha sido actualizado con datos de LinkedIn!',
  },
  navigation: {
    advisorArea: 'Área de Asesor',
    customerView: 'Vista de Cliente',
    marketplace: 'Encontrar un Asesor',
  },
  marketplace: {
    title: 'Encontrar un Asesor Financiero',
    searchPlaceholder: 'Buscar por nombre, título o especialización...',
    filters: 'Filtros',
    filterOptions: 'Opciones de Filtro',
    resetFilters: 'Restablecer Filtros',
    expertise: 'Área de Especialización',
    selectExpertise: 'Seleccionar un área de especialización',
    allExpertise: 'Todas las Áreas de Especialización',
    minRating: 'Calificación Mínima',
    anyRating: 'Cualquier Calificación',
    advisorsFound: 'asesores encontrados',
    noAdvisorsFound: 'No hay asesores que coincidan con tus criterios de búsqueda',
    viewProfile: 'Ver Perfil',
  },
  footer: {
    rights: '© 2023 CITEC AI. Todos los derechos reservados.',
  },
};
